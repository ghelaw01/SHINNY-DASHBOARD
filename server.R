...
shinyServer(function(input, output) {
  ...
  output$p1 <- renderPlot({
    if (input$graph_type == "histogram") {
      ...
      ggplot(df_country(), aes_string(x = input$continuous_variable)) +
        ...
        theme(plot.title = element_text(face="bold", color = "blue", size=18))# facet by prediction
    }
    else {
      # Boxplot
  ...
  output$p2 <- renderPlot({
    ...
    p <- ggplot(df_country(), aes_string(x = input$categorical_variable)) +
      ...
      theme(axis.text.x = element_text(angle = 45), legend.position = "bottom", plot.title = element_text(face="bold", color = "blue", size=18)) +
      scale_color_binned()# modify theme to change text angle and legend position
